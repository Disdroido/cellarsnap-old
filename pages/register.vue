  ...
  <div class="h-screen flex">
    <div class="flex w-1/2 bg-gradient-to-tr from-blue-800 to-purple-700 i justify-around items-center">
      <div>
        ...
        <NuxtLink to="/" class="block w-28 bg-white text-indigo-800 mt-4 py-2 px-3 rounded-2xl font-bold mb-2">Read More</NuxtLink>
      </div>
    </div>
    <div class="flex w-1/2 justify-center items-center bg-white">
      <form class="bg-white p-10 shadow-xl rounded-xl" @submit.prevent="handleRegister">
        ...
        <div class="flex items-center border-2 py-2 px-3 rounded-xl mb-4">
          ...
          <input v-model="email" class="pl-2 outline-none border-none" type="email" name="email" id="email" placeholder="Email Address" required />
        </div>
        ...
        <button type="submit" class="block w-full bg-indigo-600 mt-4 py-2 rounded-xl text-white font-semibold mb-2">Sign Up</button>
        <span
          class="bg-opacity-50 absolute right-8 top-8 rounded-lg bg-[#242424] p-8 px-4 py-2 text-red-500"
          v-if="errorMsg"
          >{{ errorMsg }}</span
        >
        <span class="text-sm ml-2 hover:text-blue-500 cursor-pointer"><NuxtLink to="/login">Already have an account ? Login</NuxtLink></span>
      </form>
    </div>
  </div>
</template>